...
static uint32_t PropertySet(const std::string &name, const std::string &value,
  ...
  // If init hasn't started its main loop, then it won't be handling property
  // changed messages anyway, so there's no need to try to send them.
  auto lock = std::lock_guard{accept_messages_lock};
  if (accept_messages) {
...
public:
  ...
  bool RecvString(std::string *value, uint32_t *timeout_ms) {
    ...
    // http://b/35166374: don't allow init to make arbitrarily large
    // allocations.
    if (len > 0xffff) {
    ...
    while (*timeout_ms > 0) {
      ...
      *timeout_ms = (millis > *timeout_ms) ? 0 : *timeout_ms - millis;
      ...
      if (nr < 0 && errno != EINTR) {
        ...
        // Timer rounds milliseconds down in case of EINTR we want it to be
        // rounded up to avoid slowing init down by causing EINTR with under
        // millisecond timeout.
        if (*timeout_ms > 0) {
          --(*timeout_ms);
        }
...
static uint32_t SendControlMessage(const std::string &msg,
  ...
  // We must release the fd before sending it to init, otherwise there will be a
  // race with init. If init calls close() before Release(), then fdsan will see
...
bool CheckControlPropertyPerms(const std::string &name,
                               ...
                               const ucred &cr) {
  // We check the legacy method first but these properties are dontaudit, so we
  // only log an audit if the newer method fails as well.  We only do this with
  // the legacy ctl. properties.
  if (name == "ctl.start" || name == "ctl.stop" || name == "ctl.restart") {
    // The legacy permissions model is that ctl. properties have their name
    // ctl.<action> and their value is the name of the service to apply that
    // action to.  Permissions for these actions are based on the service, so we
...
uint32_t HandlePropertySet(const std::string &name, const std::string &value,
  ...
  // sys.powerctl is a special property that is used to make the device reboot.
  // We want to log any process that sets this property to be able to accurately
  // blame the cause of a shutdown.
  if (name == "sys.powerctl") {
    std::string cmdline_path = StringPrintf("proc/%d/cmdline", cr.pid);
    std::string process_cmdline;
    std::string process_log_string;
    if (ReadFileToString(cmdline_path, &process_cmdline)) {
      // Since cmdline is null deliminated, .c_str() conveniently gives us just
      // the process path.
  ...
  // If a process other than init is writing a non-empty value, it means that
  // process is requesting that init performs a restorecon operation on the path
  // specified by 'value'. We use a thread to do this restorecon operation to
  // prevent holding up init, as it may take a long time to complete.
    ...
    *eol++ = 0;
    ...
    if (!strncmp(key, "import ", 7) && flen == 0) {
      ...
      *value++ = 0;
      ...
      if (flen > 0) {
        if (filter[flen - 1] == '*') {
          if (strncmp(key, filter, flen - 1) != 0)
            continue;
        } else {
...
// "ro.foo.*" is a prefix match, and "ro.foo.bar" is an exact match.
static bool
load_properties_from_file(const char *filename, const char *filter,
                          std::map<std::string, std::string> *properties) {
  Timer t;
...
static const char *snet_prop_key[] = {"ro.boot.vbmeta.device_state",
...
static const char *snet_prop_value[] = {"locked", // ro.boot.vbmeta.device_state
...
// If the ro.product.[brand|device|manufacturer|model|name] properties have not
// been explicitly set, derive them from ro.product.${partition}.* properties
...
// If the ro.build.fingerprint property has not been set, derive it from
// constituent pieces
...
void PropertyLoadBootDefaults() {
  ...
  // We read the properties and their values into a map, in order to always
  // allow properties loaded in the later property files to override the
  // properties in loaded in the earlier property files, regardless of if they
  // are "ro." properties or not.
  std::map<std::string, std::string> properties;
  ...
  // Weaken property override security during execution of the vendor init
  // extension
  ...
  // Restore the normal property override security after init extension is
  // executed
...
bool LoadPropertyInfoFromFile(const std::string &filename,
  ...
  ParsePropertyInfoFile(file_contents, require_prefix_or_exact, property_infos,
                        &errors);
  // Individual parsing errors are reported but do not cause a failed boot,
  // which is what returning false would do here.
...
void CreateSerializedPropertyInfo() {
  ...
  if (access("/system/etc/selinux/plat_property_contexts", R_OK) != -1) {
    ...
    // Don't check for failure here, so we always have a sane list of
    // properties. E.g. In case of recovery, the vendor partition will not have
    // mounted and we still need the system / platform properties to function.
...
static void ExportKernelBootProps() {
  ...
  for (const auto &prop : prop_map) {
    ...
    if (value != UNSET)
      InitPropertySet(prop.dst_prop, value);
  }
}
...
static void ProcessKernelDt() {
  ...
  std::unique_ptr<DIR, int (*)(DIR *)> dir(
      opendir(get_android_dt_dir().c_str()), closedir);
  if (!dir)
    return;
